...
lazy_evalution(异步计算): 
    优点: 
        效率高: 前端线程将数据丢给后端线程, 迅速返回, 而不等待后端执行完成;
    缺点:
        内存资源消耗大: 前端迅速将大量数据丢给后端, 后端开辟大量内存用于存储数据.
    使用方法:
        当内存资源不足时, 可加入nd.waitall()等同步函数, 等待后端线程返回结果后, 再继续向后端丢数据.
            同步函数有:
                1) nd.waitall()              # 最常用
                2) .asnumpy() 或 .asscalar()     # numpy和python的scalar都不支持异步计算
                3) print(后端线程的计算结果)     # 通过调用后端线程结果, 使前端等待后端
